...
| ingest-dtk melee full (16.3k fns + 56k windows, hashed) | 0.56 s |
| ingest-dtk re-run, no changes (all vectors reused) | 0.71 s |
| sweep --project melee (solvability, whole corpus) | 0.20 s |

Baseline (Python/LanceDB, same index, warm cache): find 867 ms end-to-end
(769 ms = imports), findw 3.6–12.2 s; production agent logs: findw median
...
bit-exact (self-sim 1.0); melee ingest token docs byte-identical on
